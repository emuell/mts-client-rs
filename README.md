...
This crate wraps the vendored ODDSound **client** library (`libMTSClient`) as a safe `Client`, avoiding unsafe code in user-facing APIs. The raw C API is available under `mts_client_rs::sys`.
...
Examples covering initialization, note re-tuning, reporting the tuning to the user and the MTS SysEx fallback can be found in the crate documentation, along with the real-time safety rules and platform notes:
